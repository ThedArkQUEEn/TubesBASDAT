...
class database{
    ...
    public function tambah_pesanan($jenis_laundry, $pilihan_hari, $massa_barang, $jumlah_barang, $waktu_pengambilan, $waktu_pengantaran, $alamat, $catatan, $garis_lintang, $garis_bujur, $harga_total, $status, $id, $list_satuan){
        $sql = "INSERT INTO `Order` (jenis_laundry, pilihan_hari, massa_barang, jumlah_barang, waktu_pengambilan, waktu_pengantaran, alamat, catatan, garis_lintang, garis_bujur, harga_total, status_pemesanan, id_user, list_satuan)
        VALUES (:jenis_laundry, :pilihan_hari, :massa_barang, :jumlah_barang, :waktu_pengambilan, :waktu_pengantaran, :alamat, :catatan, :garis_lintang, :garis_bujur, :harga_total, :status_pemesanan, :id_user, :list_satuan)";
        $stmt = $this-> pdo-> prepare($sql);
        $stmt->execute(array(
          ':jenis_laundry' => $jenis_laundry,
          ':pilihan_hari' => $pilihan_hari,
    ...
    public function showPesanan(){
        $sql = "SELECT id, jenis_laundry, pilihan_hari, massa_barang, jumlah_barang, waktu_pengambilan, waktu_pengantaran, alamat, catatan, garis_lintang, garis_bujur, harga_total, status_pemesanan, id_user, list_satuan FROM `Order`";
        $stmt = $this -> pdo -> query($sql);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getPesanan($name_filter){
        $sql = "SELECT jenis_laundry, pilihan_hari, massa_barang, jumlah_barang, waktu_pengambilan, waktu_pengantaran, alamat, catatan, harga_total, status_pemesanan, id_user FROM `Order` WHERE id_user = :zip";
        $stmt = $this -> pdo -> prepare($sql);
    ...
    // Method untuk mengupdate status pemesanan dan catatan pesanan
    public function updateOrderData($status, $catatan, $id) {
        try {
            $sql = "SELECT * FROM users WHERE id = :zip";
            $stmt = $this->pdo->prepare("UPDATE SatuanData SET status_pemesanan=:status, catatan=:catatan WHERE id=:id");
